...
public class DemographicGridmap2 extends PApplet{
	...
	int aggregateDist=3000;//in metres
...
//			println("end locations");
			...
			for (int i=0;i<locations.length;i++) {
				...
				if (!isEmpty) {
					...
					int x=Integer.parseInt(easting)/aggregateDist*aggregateDist+aggregateDist/2;
					int y=Integer.parseInt(northing)/aggregateDist*aggregateDist+aggregateDist/2;
					String gridKey=x+"-"+y;
		...
		try {
			for (int input=0;input<2;input++) {
				if ((LOAD_OUTPUTS && input==0) || (LOAD_BASELINE && input==1)){
					...
					if (LOAD_OUTPUTS){
						...
						int[][][] modelData=(int[][][])netcdfFile.findVariable("abundances/abuns").read().copyToNDJavaArray();
						...
						for (int locationIdx=0;locationIdx<numLocations;locationIdx++) {
							...
							int y=Integer.parseInt(northing)/aggregateDist*aggregateDist+aggregateDist/2;
							String gridKey=x+"-"+y;
							Record recordModel=gridKey2Record.get(gridKey);
							if (recordModel==null) {
								recordModel=new Record();
//								println("Made new");
								gridKey2Record.put(gridKey, recordModel);
								records.add(recordModel);
								recordModel.x=x;
								recordModel.y=y;
								if (geoBounds==null)
									geoBounds=new Rectangle2D.Float(recordModel.x,recordModel.y,0,0);
								else
									geoBounds.add(recordModel.x,recordModel.y);
							}
//							else 
//								println("Used existing");
							if (input==0)
								recordModel.resultCounts=new short[numDays][numDemographics][numStatuses];
							else if (input==1)
								recordModel.baselineCounts=new short[numDays][numDemographics][numStatuses];
							...
							for (int dayIdx=0;dayIdx<numDays;dayIdx++) 
								for (int statusIdx=0;statusIdx<numStatuses;statusIdx++) 
									for (int demographicIdx=0;demographicIdx<numDemographics;demographicIdx++)
										if (input==0)
											recordModel.resultCounts[dayIdx][demographicIdx][statusIdx]+=(short)modelData[dayIdx][locationIdx][statusIdx*numDemographics+demographicIdx];
										else if (input==1)
											recordModel.baselineCounts[dayIdx][demographicIdx][statusIdx]+=(short)modelData[dayIdx][locationIdx][statusIdx*numDemographics+demographicIdx]	;
						}
					}
				}
			}
...
//		
		...
		println(records.size()+" demographics records in "+(t1-t)/1000+" seconds");
